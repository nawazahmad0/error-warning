...
const models = ["gpt-4", "gpt-4-0613", "gpt-4-32k", "gpt-4-0314", "gpt-4-32k-0314", "gpt-3.5-turbo", "gpt-3.5-turbo-16k", "gpt-3.5-turbo-0613", "gpt-3.5-turbo-16k-0613", "gpt-3.5-turbo-0301", "text-davinci-003", "text-davinci-002", "code-davinci-002", "gpt-3", "text-curie-001", "text-babbage-001", "text-ada-001", "davinci", "curie", "babbage", "ada", "babbage-002", "davinci-002"];
...
module.exports = {
  config: {
    ...
    longDescription: {
        en: "Interact with various GPT models provided by OpenAI. This command allows users to ask questions, receive detailed answers from the AI, get lyrics of a song, as well as send images and videos.",
        vi: "Tương tác với nhiều mô hình GPT khác nhau do OpenAI cung cấp. Lệnh này cho phép người dùng đặt câu hỏi, nhận câu trả lời chi tiết từ AI, nhận lời bài hát cũng như gửi hình ảnh và video."
},
    category: "ai",
    guide: {
      en: ` 
...
ai send music [ query ] to fetch songs.
ai send shoti. 
`
    }
  },
  ...
  onChat: async function ({ api, event, args, message }) {
    try {
      ...
      if (!prompt) return message.reply("𝖧𝖾𝗅𝗅𝗈! 𝗉𝗅𝖾𝖺𝗌𝖾 𝖺𝖽𝖽 𝗒𝗈𝗎𝗋 𝗣𝗿𝗼𝗺𝗽𝘁 𝗜𝗻𝘁𝗿𝘂𝗰𝗮𝘁𝗶𝗼𝗻 𝗍𝗈 𝗀𝖾𝗍 𝖺 𝖲𝗉𝖾𝖼𝗂𝖿𝗂𝖼 𝖱𝖾𝗌𝗉𝗈𝗇𝗌𝖾. \n\n╭──🌼 \n│𝖺𝗂 ( 𝖸𝗈𝗎𝗋 𝗇𝗈𝗋𝗆𝖺𝗅 𝗉𝗋𝗈𝗆𝗉𝗍𝗌) \n│𝖺𝗂 𝗌𝖾𝗇𝖽 𝗅𝗒𝗋𝗂𝖼𝗌 ( 𝗌𝗈𝗇𝗀𝖭𝖺𝗆𝖾 ) \n│𝖺𝗂 𝗍𝖾𝗆𝗉𝗆𝖺𝗂𝗅 𝗀𝖾𝗇/𝗂𝗇𝖻𝗈𝗑 ( 𝖾𝗆𝖺𝗂𝗅 ) \n│𝖺𝗂 𝗌𝖾𝗇𝖽 𝗆𝗎𝗌𝗂𝖼 ( 𝗌𝗈𝗇𝗀𝖭𝖺𝗆𝖾 ) \n│𝖺𝗂 𝗌𝖾𝗇𝖽 𝗌𝗁𝗈𝗍𝗂 \n│𝖺𝗂 𝗌𝖾𝗇𝖽 𝗏𝗂𝖽𝖾𝗈 ( 𝗏𝗂𝖽𝖾𝗈 𝗍𝗂𝗍𝗅𝖾) \n│𝖺𝗂 𝗉𝗂𝗇 𝗊𝗎𝖾𝗋𝗒 ( 𝗍𝗂𝗍𝗅𝖾 ) - (𝗇𝗎𝗆𝖻𝖾𝗋)\n╰─────────────🌼\n\n 📝 𝗲𝘅𝗮𝗺𝗽𝗹𝗲: ai send music metamorphosis.");
...
case prompt.toLowerCase().startsWith('send music'): {
          ...
          if (!searchResults.videos.length) return message.reply("❗No song found for the given query.");
...
case prompt.toLowerCase().startsWith('tempmail'): {
          const args = prompt.split(' ').slice(1);
          if (args.length === 0) {
            await api.sendMessage({ body: "Use 'tempmail gen' to generate an email or 'tempmail inbox {email}' to check the inbox." }, event.threadID, event.messageID);
            break;
          }
          if (args[0].toLowerCase() === "gen") {
            try {
              const { data } = await axios.get("${apiEndpoint}/api/tempmail/get");
              await api.sendMessage({ body: `📮|𝗧𝗲𝗺𝗽𝗺𝗮𝗶𝗹\n━━━━━━━━━━━━━\n\nHere is your generated tempmail\n\n📍|𝗘𝗺𝗮𝗶𝗹\n➤ ${data.tempmail}` }, event.threadID, event.messageID);
            } catch (error) {
              console.error("❌ | Error", error);
              await api.sendMessage({ body: "❌|Unable to generate email address. Please try again later..." }, event.threadID, event.messageID);
            }
            ...
            try {
              ...
              await api.sendMessage({ body: inboxMessages }, event.threadID, event.messageID);
            } catch (error) {
              console.error("🔴 Error", error);
              await api.sendMessage({ body: "❌|Can't get any mail yet. Please send mail first." }, event.threadID, event.messageID);
            }
          } else {
            await api.sendMessage({ body: "❌ | Use 'tempmail gen' to generate email and 'tempmail inbox {email}' to get the inbox emails." }, event.threadID, event.messageID);
          }
          break;
        }
...
case prompt.toLowerCase().startsWith('send lyrics'): {
          const songName = prompt.split(' ').slice(2).join(' ');
          if (!songName) return message.reply("❗Please provide a song name to fetch lyrics.");

          const { data } = await axios.get(`${apiEndpoint}/api/lyrics?songName=${encodeURIComponent(songName)}`);
          if (!data.lyrics) return message.reply("❌ Lyrics not found for the given song name.");

          await message.reply(`ℹ 𝗧𝗶𝘁𝗹𝗲: ${data.title}\n\n👑 𝗔𝗿𝘁𝗶𝘀𝘁: ${data.artist}\n\n━━━━━━━━━━━━━━━\n✅ 𝗛𝗘𝗥𝗘 𝗜𝗦 𝗬𝗢𝗨𝗥 𝗟𝗬𝗥𝗜𝗖𝗦\n${data.lyrics}`);
          api.setMessageReaction("✅", event.messageID, () => {}, true);
          return;
        }
...
try {
            ...
            const apiUrl = `${apiEndpoint}/api/pinterest?query=${encodeURIComponent(
                keySearchs.trim()
            )}&limits=${searchLimit}`;
...
default: {
          ...
          const { data } = await axios.get(`${apiEndpoint}/api/gpt?prompt=${encodeURIComponent(prompt)}&model=${selectedModel}`);
